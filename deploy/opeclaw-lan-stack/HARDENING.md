...
- Configura `HOST_BIND_IP` con una IP LAN específica en vez de `0.0.0.0`.
- No publiques `vllm` ni `opeclaw` directamente; usa solo `proxy`.

## 2) Auth mínima
- Usa `REQUIRE_AUTH=1` y un `BEARER_TOKEN` largo (>=32 chars).
- Mantén `INTERNAL_OPENAI_KEY` distinta del token de proxy.
- Rota ambos secretos si se comparten fuera del equipo.
